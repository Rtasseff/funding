...
1. **Scan for calls:** run `/scan-grants` (full scan) or `/scan-grants Basque|Spain|EU` (regional). Reports are written to `SCANS/grant_scan_YYYY-MM-DD-NN.md`.
...
Grant scan output lives separately in `SCANS/` — these are internal working documents, not stakeholder deliverables.
...
REPORTS/                        <- Stakeholder-ready deliverable reports
  FUNDING_STRATEGY_REPORT.md
  program_summary_for_PIs.docx
  theory_ai_assessment_summary.docx
SCANS/                          <- Grant scan output (generated by /scan-grants)
  grant_scan_2026-03-06-01.md                   <- Basque Country grant scan (first scan)
