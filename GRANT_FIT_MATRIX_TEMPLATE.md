...
- **Readiness:** idea / partners / draft / ready / missed (deadline passed; kept for next-cycle planning)
- **Geography:** Basque / Spain / EU / Other

## Workflow

1. When a new opportunity is identified (manually or by the grant scanner), add a row.
2. Score fit using `/check-grant` or `/scan-grants` (see `GRANT_SCANNER_CLI_SPEC.md` and `BOT_MATCHING_SPEC.md`) or manually evaluate against module fiches.
